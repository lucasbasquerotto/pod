	...
	"inner:service:fastly:ips")
		...
		if [ ! -f "$output_file_outer" ] || [ "${arg_only_if_needed:-}" != 'true' ]; then
			...
			curl --fail --silent --show-error https://api.fastly.com/public-ip-list \
				| jq --raw-output '.addresses | .[]' > "$tmp_dir_inner"/ips-v4.txt
			curl --fail --silent --show-error https://api.fastly.com/public-ip-list \
