...
if [ "$tmp_is_web" = 'true' ]; then
	if [ "${var_load__enable_uploads_setup:-}" = 'true' ] &&  [ "${var_load__use_s3_storage:-}" != 'true' ]; then
...
if [ "$tmp_is_web" = 'true' ] &&  [ "${var_load__use_s3_storage:-}" != 'true' ]; then
	if [ "${var_load__enable_uploads_backup:-}" = 'true' ]; then
		tmp_uploads_src_dir='/var/main/data/mediawiki/uploads'
		...
		if [ -z "${var_load__s3_uploads__bucket_name:-}" ]; then
			errors+=("var_load__s3_uploads__bucket_name is not defined (uploads_backup)")
		fi
	...
	if [ "${var_load__enable_uploads_setup:-}" = 'true' ]; then
		...
		tmp_uploads_dest_dir='/var/main/data/mediawiki/uploads'
		tmp_uploads_tmp_base_dir='/tmp/main/tmp/mediawiki'
		...
		if [ "${tmp_restore_use_s3:-}" != 'true' ]; then
			if [ -z "${var_load__uploads_setup__restore_remote_file:-}" ]; then
				errors+=("var_load__uploads_setup__restore_remote_file is not defined")
			fi
			...
			if [ "$tmp_is_compressed_file" = 'true' ]; then
				...
				errors+=("uploads_setup: use_s3=false and is_compressed_file=false (at least 1 should be true)")
			fi
		else
			if [ -z "${var_load__s3_uploads__bucket_name:-}" ]; then
				errors+=("var_load__s3_uploads__bucket_name is not defined (uploads_setup)")
			fi
